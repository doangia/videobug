<?php
$router = new Router($_SERVER['REQUEST_URI'], 'config.php');
$router->run();
...
class Controller {
    ...
    public function __construct($lang, $controller, $action, $params) {
        ...
        {
             include 'libs/class.db.php';        

             $this->db = new db("mysql:host=".$this->config->db->host.";port=3306;dbname=".$this->config->db->database,$this->config->db->user,$this->config->db->password);
    ...
    public function route_url($action = NULL, $controller = NULL, $params = array(), $lang = NULL) {
        ...
        if($controller == 'home' && (empty($action) || $action == 'index') && empty($params)) {
            return  $this->base_url()  . implode('/', $segments);
        }
    ...
    public function base_url()       
    {
        # code...
        $root = "http://".$_SERVER['HTTP_HOST'];
        return $root .= str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
